...
def cmd_resume(path_arg: str | None) -> int:
    ...
    vocabulary: list[str] = []
    try:
        # The role pack's vocabulary makes the parse sharper, but a config that
        # cannot load yet (no facts on disk - which is exactly the case on a
        # first run) must not block parsing the resume that would fix that.
        raw = config_mod.load_pack(_role_from_config())
    ...
    facts = resume_mod.parse(source, vocabulary=vocabulary)
...
def _role_from_config() -> str | None:
    ...
    try:
        ...
        return loaded.get("role") if isinstance(loaded, dict) else None
    except Exception:
        return None
...
def main() -> int:
    ...
    parser.add_argument("--headless", action="store_true",
                        help="Run the browser headless. Naukri's bot protection usually blocks this.")
    ...
    try:
        ...
        if args.extract:
            ...
            paths.ensure()
            # Headless gets served "Access Denied" by Akamai, so this is headed.
        ...
        if isinstance(exc, (NotLoggedIn, SourceError)):
            print(f"\n  {exc}\n")
            return 2
